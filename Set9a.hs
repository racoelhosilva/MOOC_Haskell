...
workload nExercises hoursPerExercise
  ...
  | r < 10    = "Piece of cake!"
  | otherwise = "Ok."
  where r = nExercises * hoursPerExercise
...
countValid [] = 0
countValid (x:xs) = (if isValid x then 1 else 0) + countValid xs
...
sumSuccess s = case f of
                  [] -> Left "no data"
                  (x:xs) -> Right (sum $ map (\(Right x) -> x) f)
              where f = filter sumSuccess' s

sumSuccess' :: Either String Int -> Bool
sumSuccess' (Right r) = True
sumSuccess' (Left l) = False
...
instance Eq Text where
  (==) (Text a) (Text b) = f a == f b
      where f s = filter (\x -> not $ Data.Char.isSpace x) s
...
compose [] _ = []
compose (a:as) b = case lookup (snd a) b of 
                      Nothing -> compose as b
                      Just n -> (fst a, n) : compose as b
...
permute idx vals = map snd (sortBy (comparing fst) (zip idx vals))
