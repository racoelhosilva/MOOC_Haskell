...
longest :: Ord a => [[a]] -> [a]
longest x = head $ sort (takeWhile (\x -> length x == length (head ordered)) ordered)  
    where ordered = (reverse (sortBy (\a b -> compare (length a) (length b)) x))

-- longest [a] = a
-- longest (a:b:xs)
--    | length a < length b = longest (b:xs)
--    | length b < length a = longest (a:xs)
--    | otherwise           = if a < b then longest (a:xs)
--                            else longest (b:xs) 
...
incrementKey k [] = []
incrementKey k ((a,b):xs)
    | k == a    = (a, b+1) : incrementKey k xs
    | otherwise = (a, b) : incrementKey k xs
...
freqs xs = Map.fromList [(k, length (filter (\x -> x == k) xs)) | k <- nub xs]

-- freqs [] = Map.empty
-- freqs (x:xs) = Map.alter (Just . maybe 1 (+1)) x (freqs xs)

-- freqs xs = foldr (Map.alter (Just . maybe 1 (+1))) Map.empty xs
...
transfer from to amount bank
    ...
    | otherwise = receive $ send bank
        where send = Map.adjust (\v -> v - amount) from 
              receive = Map.adjust (\v -> v + amount) to
...
maxIndex arr = fst $ foldl1 (\acc p -> if snd p > snd acc then p else acc) (assocs arr)