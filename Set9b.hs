...
-- of the width (or height) n of the chess board; the naive solution with elem
...
prettyPrint s cs = prettyPrint' (1,1) s cs

prettyPrint' :: Coord -> Size -> [Coord] -> String
prettyPrint' _ 0 _ = ""
prettyPrint' (x,y) s cs
    | x > s         = "" 
    | y > s         = "\n" ++ prettyPrint' (nextRow (x,y)) s cs
    | elem (x,y) cs = "Q" ++ prettyPrint' (nextCol (x,y)) s cs
    | otherwise     = "." ++ prettyPrint' (nextCol (x,y)) s cs
...
danger (x,y) [] = False
danger (x,y) ((a,b):cs) = check || danger (x,y) cs 
    where check = sameRow (x,y) (a,b) || sameCol (x,y) (a,b) || sameDiag (x,y) (a,b) || sameAntidiag (x,y) (a,b)
...
prettyPrint2 s cs = prettyPrint2' (1,1) s cs

prettyPrint2' :: Coord -> Size -> [Coord] -> String
prettyPrint2' _ 0 _ = ""
prettyPrint2' (x,y) s cs
    | x > s           = "" 
    | y > s           = "\n" ++ prettyPrint2' (nextRow (x,y)) s cs
    | elem (x,y) cs   = "Q" ++ prettyPrint2' (nextCol (x,y)) s cs
    | danger (x,y) cs = "#" ++ prettyPrint2' (nextCol (x,y)) s cs
    | otherwise       = "." ++ prettyPrint2' (nextCol (x,y)) s cs
...
fixFirst n ((x,y):cs)
    | y > n           = Nothing
    | danger (x,y) cs = fixFirst n (nextCol (x,y):cs)
    | otherwise       = Just ((x,y):cs)
...
continue (a:as) = nextRow a : a : as

backtrack :: Stack -> Stack
backtrack (a:b:bs) = nextCol b : bs
...
step s (a:as) = case c of 
                    Just (b:bs) -> continue (b:bs)
                    Nothing -> backtrack (a:as)
                where c = fixFirst s (a:as)
