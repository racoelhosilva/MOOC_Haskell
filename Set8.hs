...
paintSolid color (Shape s) (Picture p) = Picture f
  where f coord = if s coord then color else p coord
------------------------------------------------------------------------------
...
paint (Picture fg) (Shape s) (Picture bg) = Picture f
  where f coord = if s coord then fg coord else bg coord
------------------------------------------------------------------------------
...
instance Transform Blur where
  apply (Blur) (Picture p) = Picture (\(Coord x y) -> blurred x y)
    where blurred x y = avg (map (p . toCoord) (neighbours x y))
          toCoord (a,b) = Coord a b
          neighbours x y = [(x,y), (x-1,y), (x+1,y), (x,y-1), (x,y+1)]
          avg cs = divColors (foldr sumColors (Color 0 0 0) cs) (length cs)
          sumColors (Color r1 g1 b1) (Color r2 g2 b2) = Color (r1+r2) (g1+g2) (b1+b2)
          divColors (Color r g b) n = Color (r `div` n) (g `div` n) (b `div` n)
------------------------------------------------------------------------------
