...
joinToLength n ls = [a ++ b | a <- ls, b <- ls, length (a ++ b) == n]
...
multiApp f gs a = f [g a | g <- gs]

-- multiApp f gs a = f (map (\g -> g a) gs)

-- multiApp f gs a = f (multiApp' gs a)

-- multiApp' :: [(a -> b)] -> a -> [b]
-- multiApp' [] a = []
-- multiApp' (g:gs) a = [g a] ++ multiApp' gs a
