...
fmap2 f fn = fmap (\x -> fmap f x) fn

fmap3 :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 f fn = fmap (\x -> fmap (\y -> fmap f y) x) fn
...
count e fn = length $ filter (\x -> x == e) (toList fn)
...
inBoth fn gn = filter (\x -> x `elem` toList gn) (toList fn)
...
instance Foldable List where
  ...
  foldr f start (LNode v ls) = f v (foldr f start ls)
...
instance Foldable TwoList where
  ...
  foldr f start (TwoNode a b ls) = f a (f b (foldr f start ls))
...
sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node a tl tr) = sumTree tl <> a <> sumTree tr
